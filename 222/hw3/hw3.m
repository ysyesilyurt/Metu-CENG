...
for k=1:N;
	...
	WMotors = sum(sum(-1/lambdaMotor2 * log(rand(alphaMotor, NMotors))));
	WCars = sum(sum(-1/lambdaCar2 * log(rand(alphaCar, NCars))));
	WTrucks = sum(sum(-1/lambdaTruck2 * log(rand(alphaTruck, NTrucks))));
