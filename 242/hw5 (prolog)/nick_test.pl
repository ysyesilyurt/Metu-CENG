...
configuration(COMPONENT_LIST,CONSTRAINT_LIST,PLACEMENT_LIST) :- 
			sections(RestSections), sections(AllSections), 
...
% outer_edge , WARNING -> THE CUT AFTER THE isOuterMember(Comp,CONSTRAINT_LIST) is VEEERY IMPORTANT!
...
helperOfOuter(CheckSec,[Sec|Resting],Res) :- adjacent(Sec,CheckSec), helperOfOuter(CheckSec,Resting,Res),! ; adjacent(CheckSec,Sec), helperOfOuter(CheckSec,Resting,Res),!.
...
helperOfClose(CheckSec,[Sec|Resting],Res1,Res2) :- adjacent(CheckSec,Sec), Res1 = CheckSec, Res2 = Sec ; adjacent(Sec,CheckSec), Res1 = CheckSec, Res2 = Sec.
...
% returns the other Comp and N as well.
